...
$output = $root . '/data/jurisdictions.json';
...
$json = json_encode($entries, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_PRESERVE_ZERO_FRACTION);
if ($json === false) {
    fwrite(STDERR, "Failed to encode dataset as JSON.\n");
    exit(1);
}
...
if ($json === null) {
    fwrite(STDERR, "Failed to adjust JSON indentation.\n");
    exit(1);
}

if (file_put_contents($output, $json . "\n") === false) {
    fwrite(STDERR, "Failed to write dataset to {$output}.\n");
    exit(1);
}

printf("Wrote %d jurisdictions to %s\n", count($entries), $output);
