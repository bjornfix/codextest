...
- **On-the-fly updates** – A token-protected management form lets authorised
  operators add or edit jurisdictions directly from the dashboard; changes
  persist to the JSON store instantly.
...
4. (Optional) Enable authenticated in-browser edits by defining a dataset
   update token before launching the server:

   ```bash
   export DATASET_UPDATE_TOKEN="choose-a-secret"
   ```

   Supply the same token in the management form whenever you save changes.

Use the **Maintain the dataset** section (`#manage`) to update or add
jurisdictions without leaving the app. When `DATASET_UPDATE_TOKEN` is set,
providing the token in the form writes the changes straight back to the flat
JSON file; otherwise, the dashboard stays read-only and you can edit the JSON
manually.
...
- **In-browser edits:** Define `DATASET_UPDATE_TOKEN` before starting the server,
  then scroll to the **Maintain the dataset** panel and submit the form with
  that token to add a new jurisdiction or overwrite an existing record.
  Multi-line fields accept one list item per line and persist back to
  `data/jurisdictions.json`. Without a token configured, keep editing the JSON
  file directly.
